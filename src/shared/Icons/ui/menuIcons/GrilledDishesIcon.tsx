import { Icon } from '@chakra-ui/react';

export const GrilledDishesIcon = () => (
    <Icon viewBox='0 0 24 24' fill='none' boxSize={{ base: '16px', md: '24px' }}>
        <path fill='url(#prefix__pattern0_5002_136732)' d='M0 0h24v24H0z' />
        <defs>
            <pattern
                id='prefix__pattern0_5002_136732'
                patternContentUnits='objectBoundingBox'
                width='1'
                height='1'
            >
                <use
                    xlinkHref='#prefix__image0_5002_136732'
                    transform='matrix(.00781 0 0 .00781 .125 .125)'
                />
            </pattern>
            <image
                id='prefix__image0_5002_136732'
                width='96'
                height='96'
                preserveAspectRatio='none'
                xlinkHref='data:image/png;base64,'
            />
        </defs>
    </Icon>
);
